...
class ODENet(nn.Module):
    ...
    def save(self, fp):
        ...
        # Safely get the directory, the filename (stem), and the extension (suffix)
        directory = base_path.parent
        stem = base_path.stem
        suffix = base_path.suffix

        # Create new, correct paths
        prod_path = directory / f"{stem}_prods{suffix}"
        sum_path = directory / f"{stem}_sums{suffix}"
        alpha_comb_path = directory / f"{stem}_alpha_comb{suffix}"
        gene_mult_path = directory / f"{stem}_gene_multipliers{suffix}"
        
        # Save each component
        torch.save(self.net_prods, str(prod_path))
        torch.save(self.net_sums, str(sum_path))
        torch.save(self.net_alpha_combine, str(alpha_comb_path))
        torch.save(self.gene_multipliers, str(gene_mult_path))
    ...
    def load_model(self, fp):
        ...
        # Create the paths to the component files
        prod_path = directory / f"{stem}_prods{suffix}"
        sum_path = directory / f"{stem}_sums{suffix}"
        alpha_comb_path = directory / f"{stem}_alpha_comb{suffix}"
        gene_mult_path = directory / f"{stem}_gene_multipliers{suffix}"
        ...
        # Load each component
        self.net_prods = torch.load(str(prod_path))
        self.net_sums = torch.load(str(sum_path))
        self.gene_multipliers = torch.load(str(gene_mult_path))
        self.net_alpha_combine = torch.load(str(alpha_comb_path))
