...
# === Repo root ===
#TODO put this in the config- make it easier to the end user - have them only adjust the config file and run for their use case
#rename the Path() in the computer/cluster/device -- but it's good to just change once
...
def plot_MSE(epoch_so_far, training_loss, validation_loss, true_mean_losses, true_mean_losses_init_val_based, prior_losses, img_save_dir, output_root_dir):
    ...
    #REPROD
    np.savetxt(output_root_dir / '{}full_loss_info.csv'.format(output_root_dir), np.c_[training_loss, validation_loss, true_mean_losses, true_mean_losses_init_val_based], delimiter=',')
